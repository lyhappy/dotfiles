#!/usr/bin/env bash
  ...
  tmux set-option -g status-right " ${left_sep}#[bg=${black},reverse] #(whoami)@#h #[bg=${light_purple}, fg=${dark_gray}] %Y-%m-%d %H:%M:%S "
  ...
  tmux set-window-option -g window-status-current-format \
    "${right_sep}#[fg=${black}] #I #W ${right_alt_sep} #P #[fg=${dark_gray},reverse]${right_sep}"
